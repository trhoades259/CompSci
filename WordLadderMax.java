...
class WordLadderMax {
	public static void main(String[] args) {
		...
		String uppercaseStart = "", uppercaseEnd = "";
		for(int charIndex = 0; charIndex < wordStart.length(); charIndex++) {
			uppercaseStart+=Character.toString(Character.toUpperCase(wordStart.charAt(charIndex)));
			uppercaseEnd+=Character.toString(Character.toUpperCase(wordEnd.charAt(charIndex)));
		}
		wordStart = uppercaseStart;
		wordEnd = uppercaseEnd;
		...
		int lengthMax = 0;
		ArrayList<String> ladderMax = new ArrayList<String>();
		for(;attempts>0;attempts--) {
			...
			if(ladder.size()<lengthMax || lengthMax==0) {
				ladderMax = ladder;
				lengthMax = ladder.size();
			}
		}

		System.out.println();
		for(int ladderIndex=0; ladderIndex<ladderMax.size(); ladderIndex++) System.out.println(ladderMax.get(ladderIndex));
		System.out.println("Ladder length: "+ladderMax.size());
